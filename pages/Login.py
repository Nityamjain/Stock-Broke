...
import streamlit.components.v1 as components  # For embedding JS
                ...
                if user_email:
                    try:
                        ...
                        if 'not found' in str(e).lower():
                            ...
                            # Generate link server-side for fallback
                            ...
                            st.info(f"Verification URL: ```{link}``` (Check email or paste to verify.)")
                            return None
                        else:
                            raise
        return None
...
def send_verification_js_component(email, uid):
    """Client-side JS component to send verification email using Firebase JS SDK."""
    firebase_config = st.secrets["FIREBASE_WEB"]
    config_json = json.dumps(firebase_config)
    
    # HTML/JS: Initializes Firebase client-side and sends email
    html = f"""
    ...
    <script>
        ...
        // Sign in with custom token (generated server-side for security)
...
def signup_callback():
    ...
    try:
        ...
        # Generate custom token for client-side auth (secure way to init user)
        ...
        # Send verification via client-side JS
...
if st.session_state.singout:
    ...
    if st.button("SignOut", key=f"signout_{st.session_state.usermail}"):
        logout_callback()
